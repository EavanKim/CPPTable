...
private:
	bool CheckRange(void* _ptr)
	{
		INTPTR checkTarget = ParsePtr(_ptr);
		INTPTR offset = checkTarget - ParsePtr(m_data);
		if (-1 < offset)
		{
			offset = totalSize - offset;
			if (-1 < offset) // 끝과 같으면 Chunk Memory 관련 Memory Overrun 이 발생했습니다. 실패로 던집니다.
			{
				return true;
			}
		}
